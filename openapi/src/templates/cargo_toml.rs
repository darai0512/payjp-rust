...
fn get_serialization_feature(deps: &[Crate], kind: &str) -> String {
    let mut features = vec![format!("async-payjp-types/{kind}")];
    for dep in deps {
...
pub fn gen_crate_toml(krate: Crate, crate_deps: Vec<Crate>, crate_features: Vec<String>) -> String {
    ...
    // Dependencies only needed for libraries which implement requests
    let request_deps = if krate == Crate::SHARED {
        ""
    } else {
        r#"async-payjp-client-core = {path = "../../async-payjp-client-core"}"#
    };
    ...
    formatdoc! {
        ...
        async-payjp-types = {{path = "../../async-payjp-types"}}
        {request_deps}
