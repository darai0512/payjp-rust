...
fn get_req_details<'a>(
    ...
    let item = &spec.get_request(&op.path).unwrap().as_item().expect("Expected requests to be items");
    ...
    for param in operation.parameters.iter().chain(item.parameters.iter()) {
        let p = match param {
            ReferenceOr::Reference { reference: r } => {
                let path = ComponentPath::from_reference(r);
                let params = &spec.components().parameters;
                let key = path.as_ref().trim_start_matches("#/components/parameters/");
                params.get(key).unwrap().as_item().expect("invalid")
            },
        ...
        match p {
            ...
            _ => bail!("Unexpected parameter type {p:?}"),
        }
    }
...
pub fn parse_requests(
    ...
    for req in &req_details {
        let method_name = &method_names[&RequestKey::from_op(req)];
        ...
        let return_type =
            Inference::new(&return_ident).spec(spec).required(true).infer_schema_or_ref_type(req.returned);

        let builder_ident = RustIdent::joined(&req_ident, "Builder");
        let param_inference = Inference::new(&req_ident).spec(spec).can_borrow(false).required(true);
